    ...
    def native_value(self) -> StateType | date | datetime | Decimal:
        match self.model:
            case "SENSOR_AIR_TEMP":
                return self.coordinator.data[BACKYARD_SYSTEM_ID]['omni_telemetry']['@airTemp']
            case "SENSOR_WATER_TEMP":
                telem_temp = self.coordinator.data[self.bow_id]['omni_telemetry']['@waterTemp']
                return telem_temp if int(telem_temp) != -1 else None
            case "SENSOR_SOLAR_TEMP":
                return self.coordinator.data[self.heater_system_id]['omni_telemetry']['@temp']
